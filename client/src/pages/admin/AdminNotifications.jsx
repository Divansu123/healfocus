...
export default function AdminNotifications({ onClose }) {
  ...
  const markAll = async () => {
    try {
      await Promise.all(notifs.filter(n => !n.read).map(n => adminApi.markNotificationRead(n.id)))
      ...
      <div className="relative bg-white w-full max-w-sm rounded-t-3xl lg:rounded-3xl lg:mt-16 lg:mr-4 max-h-[85vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100">
          <div>
            ...
            {unread > 0 && <p className="text-xs text-red-500 font-bold">{unread} unread</p>}
          </div>
            ...
            <div key={n.id} onClick={() => !n.read && markRead(n.id)}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  {!n.read && <span className="w-2 h-2 rounded-full bg-red-500 flex-shrink-0 ml-2" />}
                </div>
