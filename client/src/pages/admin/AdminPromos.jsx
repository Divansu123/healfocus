...
export default function AdminPromos() {
  ...
  const [form, setForm] = useState({ title:'',desc:'',type:'Discount',discount:'',validTill:'',applicableTo:'all',color:GRADIENTS[0],active:true })
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await adminApi.updatePromotion(editing.id, form)
      ...
      setForm({ title:'',desc:'',type:'Discount',discount:'',validTill:'',applicableTo:'all',color:GRADIENTS[0],active:true })
      load()
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { setEditing(null); setForm({ title:'',desc:'',type:'Discount',discount:'',validTill:'',applicableTo:'all',color:GRADIENTS[0],active:true }); setModal(true) }}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-xs font-bold bg-white/20 px-2 py-0.5 rounded-full">{p.discount}</span>
                    {p.validTill && <span className="text-[10px] opacity-70">Till {fmtDate(p.validTill)}</span>}
                  </div>
                </div>
                <div className="flex gap-1 ml-2">
                  <button onClick={() => { setEditing(p); setForm({title:p.title,desc:p.description||p.desc||'',type:p.type||'Discount',discount:p.discount||'',validTill:p.validTill||'',applicableTo:p.applicableTo||'all',color:p.color||GRADIENTS[0],active:p.active}); setModal(true) }}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={(editing?'✏️ Edit':'🎁 New') + ' Promotion'}>
        ...
        <FormGroup label="Color">
          <div className="flex gap-2 flex-wrap">
            {GRADIENTS.map(g => (
              <button key={g} onClick={() => setForm(f=>({...f,color:g}))}
                className={`w-8 h-8 rounded-lg border-2 ${form.color===g?'border-primary-600':'border-transparent'}`} style={{background:g}} />
            ))}
          </div>
        </FormGroup>
